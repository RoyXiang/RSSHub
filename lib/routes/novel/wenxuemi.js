...
module.exports = async (ctx) => {
    ...
    const updated_at = new Date($('.block_txt2>p').eq(5).text().substr(3));
    const offset = updated_at / 1000 - parseInt(list.eq(0).find('a').attr('href').split('/').pop().replace('.html', ''));
            ...
            list.toArray().map(async (item) => {
                ...
                if (value) {
                    ...
                    resultItem = {
                        ...
                        link: link,
                        pubDate: new Date((parseInt(link.split('/').pop().replace('.html', '')) + offset) * 1000).toUTCString(),
                    };
                    if (res.text().includes(loading) === false) {
                        ctx.cache.set(link, JSON.stringify(resultItem));
                    } else {
                        resultItem.description = loading;
                    }
                }
    ...
    ).filter((item) => item.description !== loading);
